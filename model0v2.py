...
# =======================================================================
# 🚨 MODELO 0: PURELY REACTIVE ENGINE - VERSION 0.9.1 (FAIR COMPARISON) 🚨
# Goal: Fixed high speed, no cognitive logic. Equal physics to M1/M3.2.
# =======================================================================
...
META_PARAMS = {
    ...
    "inflation": 0.85,
    "resolution": 0.1,
    "f_loc": 5, "f_glob": 20, "initial_battery": 100.0,
...
def save_to_excel():
    ...
    report = pd.DataFrame({
        "Metric": ["Avg SE Mark", "Total Spills", "Total Collisions", "Spill Rate (per frame)"],
        "Result": [
    ...
    config = pd.DataFrame({
        "Parameter": ["Model", "Speed", "Inflation", "Resolution",
                      "Freq_Loc", "Freq_Glob", "Initial_Battery_%",
                      "Init_Uncertainty", "Goal_Threshold",
                      "Total_Runs", "Room_Size_at_End", "Step_Size"],
        "Value": ["M0-REACTIVE", META_PARAMS["speed"], META_PARAMS["inflation"],
                  META_PARAMS["resolution"], META_PARAMS["f_loc"], META_PARAMS["f_glob"],
                  META_PARAMS["initial_battery"], META_PARAMS["init_uncertainty"],
                  META_PARAMS["goal_threshold"], total_runs_to_execute, ROOM_SIZE, STEP_SIZE]
    })

    with pd.ExcelWriter(f"model0_final_v091_fair_{date_str}.xlsx") as writer:
    ...
    print(f"✅ EXCEL GENERATED. M0 Fair Score: {np.mean(run_scores):.2f}")
...
def draw_graphics():
    ...
    ax.text(0.1, ROOM_SIZE + 0.6, "AGENT: M0-REACTIVE (STATIC)", color='blue', fontweight='bold', fontsize=12)
    ax.text(0.1, ROOM_SIZE + 0.2, f"Target Speed: {META_PARAMS['speed']} m/s | Pure Odometry Physics", fontsize=10)
...
btn = Button(plt.axes([0.02, 0.25, 0.15, 0.06]), '▶ Start Fair M0', color='deepskyblue')
